...
/**
 * aep-dataLayer v2.0
 * Enhanced ecommerce tracking for Shopify with improved performance and reliability
 */
(function() {
  ...
  class aep_dataLayer {
    constructor() {
      ...
      // dacă Add to Cart redirecționează la /cart
      this.isAddToCartRedirect = false;
      // dacă increment/decrement/remove în cart NU reîncarcă pagina
      this.isAjaxCartIncrementDecrement = true;

      // === date de bază din Shopify ===
      this.cart = {{ cart | json }};
      ...
      // Customer data pentru parametri
      this.customerData = {
        isLoggedIn: false,
        totalSpent: 0,
        ordersCount: 0
      ...
      // Page type pentru dynx_pagetype și ecomm_pagetype
    ...
    getPageType() {
      {% if template contains 'product' %}
        return { dynx: 'offerdetail', ecomm: 'product' };
      {% elsif template contains 'collection' %}
        return { dynx: 'category', ecomm: 'category' };
      {% elsif template contains 'cart' %}
        return { dynx: 'cart', ecomm: 'cart' };
      {% elsif template contains 'search' %}
        return { dynx: 'searchresults', ecomm: 'searchresults' };
      {% elsif template == 'index' %}
        return { dynx: 'home', ecomm: 'home' };
      {% else %}
    ...
    eventConsole(eventName, eventData) {
      const css1 = 'background: DarkMagenta; color: #fff; font-size: normal; border-radius: 3px 0 0 3px; padding: 3px 4px;';
      const css2 = 'background-color: DarkOrchid; color: #fff; font-size: normal; border-radius: 0 3px 3px 0; padding: 3px 4px;';
        ...
        document.addEventListener('pointerdown', (event) => {
          ...
          if(target) {
            ...
            if(newQuantity && (line || cart_id)) {
              ...
              if(newQuantity < item.quantity) { event = 'remove_from_cart'; }
  ...
  document.addEventListener('DOMContentLoaded', function() {
    try{
      window.aepDataLayerInstance = new aep_dataLayer();
    }catch(error) {
      console.log(error);
    }
  });
